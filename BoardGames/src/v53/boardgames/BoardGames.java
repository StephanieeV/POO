package v53.boardgames;

import java.util.Scanner;
import v53.boardgames.game.Action;
import v53.boardgames.game.Board;
import v53.boardgames.game.GameState;
import v53.boardgames.game.IGame;
import v53.boardgames.game.crush.Crush;
import v53.boardgames.game.fill.Fill;
import v53.boardgames.io.FileUtils;
...
public class BoardGames {
    ...
    private void start(IGame game) {
        GameState state = new GameState(new Board(), 0);
        Player players[] = game.players[0];
  
        while (!state.isEnd()) {
            System.out.println(state.getBoard().display(false));
            final Action action = players[state.current].play();
